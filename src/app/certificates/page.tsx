...
export default async function CertificatesPage() {
  ...
  const completedProgress = moduleIds.length
    ? await prisma.progress.findMany({
        where: { userId: session.user.id, moduleId: { in: moduleIds }, completed: true },
      })
    : [];
  ...
  const courseStats = courses.map((course) => {
    ...
    return { course, total, completed, isComplete: total > 0 && completed === total };
  });
    ...
    courseStats
      ...
      .map((c) =>
        prisma.certificate.upsert({
          where: { userId_courseId: { userId: session.user.id, courseId: c.course.id } },
          update: {},
      ...
      <div className="mx-auto w-full max-w-2xl px-6 py-10">
        ...
        <div className="mt-6 flex flex-col gap-4">
          {certificates.map((cert) => (
            <div key={cert.id} className="overflow-hidden rounded-xl border">
              <div className="border-b bg-muted/40 p-6 text-center">
                <Award className="mx-auto h-7 w-7 text-amber-500" aria-hidden="true" />
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  Diberikan kepada {session.user.name}, {dateFormatter.format(cert.issuedAt)}
                </p>
              </div>
              <div className="flex gap-2 p-3">
                <Button variant="outline" size="sm" disabled className="flex-1 gap-1.5 opacity-60">
          ...
          {lockedCourses.map(({ course, completed, total }) => (
            <div key={course.id} className="flex items-center gap-4 rounded-xl border bg-muted/20 p-4">
