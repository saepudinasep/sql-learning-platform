...
export default async function SettingsPage() {
  ...
  const [account, completedCount, certificateCount, streak] = await Promise.all([
    prisma.account.findFirst({ where: { userId: session.user.id }, select: { provider: true } }),
  ...
  const providerLabel = account ? PROVIDER_LABELS[account.provider] ?? account.provider : null;
      ...
      <div className="mx-auto w-full max-w-xl px-6 py-10">
        ...
        <div className="mt-6 rounded-xl border p-5">
          ...
          {providerLabel && (
            <p className="mt-4 text-xs text-muted-foreground">
              Masuk lewat akun <span className="font-medium">{providerLabel}</span>. Nama, email, dan
        ...
        <div className="mt-4 rounded-xl border bg-muted/30 p-5">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Semua modul saat ini bisa diakses gratis. Paket berbayar akan tersedia di kemudian hari.
          </p>
        </div>
        ...
        <div className="mt-8 rounded-xl border border-destructive/30 p-5">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Tindakan ini akan menghapus akun beserta seluruh progres, sertifikat, dan riwayatmu secara
            permanen. Belum bisa dilakukan dari sini — fitur ini masih dalam pengembangan.
          </p>
          <Button variant="outline" size="sm" disabled className="mt-3 text-destructive">
