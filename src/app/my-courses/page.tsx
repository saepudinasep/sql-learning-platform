...
export default async function MyCoursesPage() {
  ...
  const coursesWithStats = courses.map((course) => {
    ...
    const nextModule =
      course.modules.find(
        (m) =>
          m.accessLevel === "FREE" && !progressByModuleId.get(m.id)?.completed,
      ) ?? course.modules[0];
        ...
        {myCourses.length === 0 ? (
          <div className="mt-8 rounded-xl border p-6 text-center">
            ...
            <p className="mt-3 text-sm text-muted-foreground">
              Belum ada progres tercatat. Mulai dari modul pertama di salah satu
              kursus di bawah.
            </p>
          </div>
        ) : (
          <div className="mt-6 flex flex-col gap-3">
            {myCourses.map(
              ({ course, completedCount, progressPct, nextModule }) => (
